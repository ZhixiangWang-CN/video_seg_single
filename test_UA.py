...
def train(parameters):
    ...
    with torch.no_grad():
        ...
        D95 = mean_confidence_interval(Dise_list_class)

        H95 = mean_confidence_interval(Haus_list_class)
        ...
        y = np.ones(n)
        plt.subplot(1,2,1)
        plt.scatter(y, Dise_list, c='blue', alpha=0.5)

        plt.boxplot([Dise_list], labels=['UA'], showfliers=False)
        plt.title("UA Dice")
        plt.ylim((0, 1))
        plt.subplot(1,2,2)
        plt.hist(Dise_list,color='blue' ,bins=20,edgecolor="black",alpha=0.7)
        ...
        y = np.ones(n)
        plt.subplot(1,2,1)
        plt.scatter(y, cost_time_list,c='blue')
        plt.boxplot(cost_time_list, labels=['Time cost'], showfliers=False)
        plt.title("UA Time")
        # plt.ylim((0, 1))
        plt.subplot(1, 2, 2)
        plt.hist(cost_time_list, color='blue', bins=20, edgecolor="black", alpha=0.7)
        plt.title("UA Histogram")
        plt.savefig(pic_save_path + "Time.tif")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process some integers.')
    parser.add_argument("--parameters",type=str,default='3DUNet_UA.json', help="parameters json")
